...
function validateEventPayload(body) {
    ...
    const categoryIdRaw = normalizeText(body.category_id || body.categoryId);
    const category_id = categoryIdRaw && /^[A-Za-z0-9_-]{1,64}$/.test(categoryIdRaw)
        ? categoryIdRaw
        : null;
...
router.post('/', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await query.run(
            `INSERT INTO events (id, calendar_id, creator_id, title, location, allday, start_time, end_time, color, category_id, memo, visibility, hp_consumption, motivation_consumption, recurrence, event_type, reminder_minutes, notify_at_start, task_deadline_notify, mail_reminder_enabled, mail_to, mail_subject, mail_remind_at, mail_sent)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                eventId,
                targetCalendarId,
...
router.put('/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await query.run(
            `UPDATE events 
             SET calendar_id = ?, title = ?, location = ?, allday = ?, start_time = ?, end_time = ?, color = ?, category_id = ?, memo = ?, visibility = ?, hp_consumption = ?, motivation_consumption = ?, recurrence = ?, event_type = ?, reminder_minutes = ?, notify_at_start = ?, task_deadline_notify = ?, mail_reminder_enabled = ?, mail_to = ?, mail_subject = ?, mail_remind_at = ?, mail_sent = ?
             WHERE id = ?`,
            [
                targetCalendarId,
                eventData.title,
...
router.post('/copy-paste', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        for (const eventId of eventIds) {
            ...
            await query.run(
                `INSERT INTO events (id, calendar_id, creator_id, title, location, allday, start_time, end_time, color, category_id, memo, visibility, hp_consumption, motivation_consumption, recurrence)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [newEventId, event.calendar_id, userId, event.title, event.location, event.allday, startStr, endStr, event.color, event.category_id || null, event.memo, event.visibility, event.hp_consumption, event.motivation_consumption, event.recurrence]
            );
